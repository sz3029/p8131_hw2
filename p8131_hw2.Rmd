...
# Bind columns
out.logit = cbind(exp(CI.logit)[-1,,drop = FALSE], 
                  coef(summary(fit.logit))[-1,4,drop = FALSE], 
                  fit.logit$deviance)
out.probit = cbind(exp(CI.probit)[-1,,drop = FALSE], 
                   coef(summary(fit.probit))[-1,4,drop = FALSE], 
                   fit.probit$deviance)
out.cloglog = cbind(exp(CI.cloglog)[-1,,drop = FALSE], 
